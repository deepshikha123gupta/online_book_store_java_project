...
%>  
  ...
  <option>security question1</option>
  <options>security question2</option>
  <option>security question3</option>
  <option>security question4</option>
  <option>security question5</option>
</select>  
           <input type="text" class="form-control" id="t6" name="t6" placeholder="Password" required >    
             </div>
 ...
 res1=st.executeQuery("select username from detail where username='"+un+"'"); 
 while(res1.next()){
 c++; 
 }
      if(pass.equals(cpass) && c==0)
      {
 res=st.executeUpdate("insert into detail(name,email,username,password) values('"+n+"','"+e+"','"+un+"','"+pass+"')");
